 ...
 *  2017EET2296
 *
 *  Utility function definations used in the code
 */
...
int new_server(uint32_t inaddr, uint16_t port, int backlog) {
    ...
    // Listen for incoming clients. Backlog indicates number of concurrent clients
...
// Send path
// Return status: -1 error opening file, -2 send file error, -3 close file error
...
int recv_file(int peer, FILE *f) {
    ...
    int n;
    // While number of bytes recieved is greater than 0, keep recieving
 ...
 * This function internally calls recv_file to actually transfer the file
...
int parse_number(const char *buf, uint32_t *number) {
    ...
    for (int i = 0; buf[i] != 0 && i < BUF_SIZE; i++) {
        if (!isdigit(buf[i])) {
            if (f >= 0) {
